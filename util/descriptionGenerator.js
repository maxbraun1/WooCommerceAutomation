function descriptionGenerator(item) {
  ...
  html = html + "<table>";
  if (item.desc) {
    html = html + "<tr><td>Description</td><td>" + item.desc + "</td></tr>";
  }
  if (item.caliber) {
    html = html + "<tr><td>Caliber</td><td>" + item.caliber + "</td></tr>";
  }
  if (item.action) {
    html = html + "<tr><td>Action</td><td>" + item.action + "</td></tr>";
  }
  if (item.capacity) {
    html = html + "<tr><td>Capacity</td><td>" + item.capacity + "</td></tr>";
  }
  if (item.model) {
    html = html + "<tr><td>Model</td><td>" + item.model + "</td></tr>";
  }
  ...
  html =
    html +
    "<span style='color:red;font-weight:bold'>ALL FIREARMS MUST BE SHIPPED TO A CURRENT FEDERAL FIREARMS LICENSED DEALER (FFL DEALER)</span><br>";

  html =
    html +
    "<span>It is the buyer's responsibility to verify that this <b>" +
    item.manufacturer +
    " " +
    item.model +
    " " +
    item.upc +
    "</b> and its Accessories (Magazine, Etc.), are compliant <a href='https://www.statefirearmlaws.org/state-state-firearm-law-data' target='_blank'>in your state</a>.</span><br>";

  html =
    html +
    "<span>It is the buyer's responsibility to contact the FFL dealer to ensure they accept transfers, also a copy of their FFL is required to be emailed to <a href='mailto:sales@secguns.com'>sales@secguns.com</a>, before purchasing a firearm. Your order will not be fulfilled without a copy of their FFL.</span><br>";

  html =
    html +
    "<span>If you have any questions, please check out our <a href='https://secguns.com/faq/'><span style='color: #3366ff;'>FAQs</span></a> or <a href='https://secguns.com/buying-a-gun-online/'><span style='color: #3366ff;'>Buying A Gun Online</span></a> page.</span><br>";
