...
async function getInventoryFile() {
  ...
  try {
    ...
    await client.downloadTo(
      "files/davidsons_inventory.csv",
      "davidsons_inventory.csv"
    );
    await client.downloadTo(
      "files/DAV-inventory-with-MAP.txt",
      "DAV-inventory-with-MAP.txt"
    );
    await client.downloadTo(
      "files/davidsons_quantity.csv",
...
async function getInventory() {
  ...
  let DavidsonsQuantities = csvToJson
    .fieldDelimiter(",")
    .getJsonFromCsv("files/davidsons_quantity.csv");
  ...
  let products = csvToJson
    .fieldDelimiter(",")
    .getJsonFromCsv("files/davidsons_inventory.csv");
  ...
  let items = products.map((item) => {
    if (item.Quantity == "A*") {
      ...
      let quantityInfo = DavidsonsQuantities.find(
        (product) =>
          parseInt(product.UPC_Code) == parseInt(item.UPCCode.replace("#", ""))
      );
      if (quantityInfo) {
        item.Quantity =
          parseInt(quantityInfo.Quantity_NC.replace("+", "")) +
          parseInt(quantityInfo.Quantity_AZ.replace("+", ""));
      } else {
        item.Quantity = parseInt(item.Quantity);
      }
    }

    let info = productMAPs.find(
      (product) => product.upc == parseInt(item.UPCCode.replace("#", ""))
    );
    let map;
    if (!info) {
      map = 0;
...
function postOnGunBroker(item) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (title.length > 75) {
        title =
          item.Manufacturer + " " + item.ModelSeries + " | " + item.UPCCode;
        if (title.length > 75) {
          return;
        }
      }
...
async function postAllListings(listings, limit) {
    ...
    if (alreadyPosted) {
      console.log(
        chalk.bold.blue.bgWhite(
          " Davidsons Item " + count + " / " + listings.length + " "
        ) + chalk.bold.yellow(" [" + item.UPCCode + "] Item already posted.")
      );
    } else {
  ...
  console.log(
    chalk.bold.green(
      "Davidsons postings complete. " + countPosted + " listings posted."
    )
  );
  return countPosted;
}
