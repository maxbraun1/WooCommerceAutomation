  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (let item of data) {
      if (parseInt(item.ITYPE._text) == 1 || parseInt(item.ITYPE._text) == 2) {
        if (
          (await categories[item.CATID._text]) != "LOWERS" &&
          (await categories[item.CATID._text]) != "SPECIALTY"
        ) {
          // Skip if undefined
          ...
          newItem.img =
            "https://media.server.theshootingwarehouse.com/large/" +
            item.PICREF._text +
            ".jpg";
          newItem.category = await categories[item.CATID._text];
          newItem.mfgPartNumber = item.MFGINO._text.trimEnd();
          newItem.series = item.SERIES._text
            ? item.SERIES._text.trimEnd()
            : undefined;
...
async function postAllListings(listings, limit) {
    ...
    if (alreadyPosted) {
      console.log(
        chalk.bold.blue.bgWhite(
          " Sports South Item " + count + " / " + listings.length + " "
  ...
  console.log(
    chalk.bold.green(
      "Sports South postings complete. " + countPosted + " listings posted."
    )
  );
  return countPosted;
}
